#!/bin/zsh

## coding analysis
lines_coded() {
    perl -ne'print unless /^\s*$/ || /^\s*(?:#|\/\*|\*)/' "$@" | wl
}
lines_commented() {
    perl -ne'print if /^\s*(?:#|\/\*|\*)/' "$@" | wl
}

## add a blank space to the Mac OSX Dock
add_spacer_to_dock () {
  defaults write com.apple.dock persistent-apps -array-add '{tile-data={}; tile-type="spacer-tile";}'
  killall Dock
}

## see the module and function of perl code in a directory
## e.g. $ perl_dir_structure
perl_dir_structure() {
    find . -name "*_Test.pm" -prune -o -type f -print0 \
    | xargs -0 perl -nle'
        BEGIN{ $last_p = undef} 
        $last_p = undef if eof;
        ($p) = /^package (.+);/;
        $last_p = $p if $p;
        ($s) = /^sub (\w+)\b/;
        next unless $s;
        $all{$last_p}{$s}++;
        END{ for $p (keys %all) { print "\n$p"; print "\t$_" foreach keys %{$all{$p}} }}'
}

tailgrep() {
	tail -f "$1" | grep --line-buffered "$2"
}

v() {
	wl "$1"
	l "$1"
	head "$1"
}

mkc() {
    mkdir -p "$1" && cd "$1"
}

# settitle "window terminal title"
settitle()
{
    printf "\e]2;$*\a\e]1;$*\a\e]0;$*\a";
}


notify_mac() {
    osascript \
      -e 'on run argv' \
      -e 'display notification (item 1 of argv) with title "Emergency" sound name "Glass"' \
      -e 'end run' \
      -- "$*"
}

cat() {
  if [[ "$1" == *.md ]]; then
    if have glow; then
      glow "$@"
    elif have mdcat; then
      mdcat "$@"
    else
      command cat "$@"
    fi
  else
    command cat "$@"
  fi
}

# don't add jrnl entries to history
function zshaddhistory() {
    emulate -L zsh
    if [[ "$1" == jrnl || "$1" == jrnl\ * ]]; then
        return 1
    fi
}

# Track last directory to avoid repeat triggers
typeset -g LAST_NOTE_DIR=""
function show_note_on_cd() {
  local note_file=".onenter"

  if [[ -f "$note_file" && "$PWD" != "$LAST_NOTE_DIR" ]]; then
    echo
    cat "$note_file"
    echo
    LAST_NOTE_DIR="$PWD"
  fi
}
autoload -U add-zsh-hook
add-zsh-hook chpwd show_note_on_cd
show_note_on_cd # Also run once on shell startup (for initial dir)
